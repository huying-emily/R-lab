...
yieldDat = read.table("Rlab10_yields.txt",header=T)
...
stocks = read.csv("Rlab10_Stock_FX_Bond_2004_to_2005.csv",header=T)
attach(stocks)
stocks_subset=as.data.frame(cbind(GM_AC,F_AC,UTX_AC,MRK_AC))
FF_data = read.table("Rlab10_FamaFrenchDaily.txt",header=T)
...
dat = read.csv("Rlab10_Stock_FX_Bond.csv")
